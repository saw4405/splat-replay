...
class TemplateMatcher:
    ...
    def match(self, frame: np.ndarray, use_grayscale: bool = True) -> Tuple[bool, Optional[Tuple[int, int]]]:
        """
        フレーム内でテンプレートを探し、一致したかどうかを返す。

        :param frame: 検索対象のフレーム。
        :param use_grayscale: グレースケールでテンプレートマッチングを行うかどうか。
        ...
        result = cv2.matchTemplate(
            frame, template, cv2.TM_CCOEFF_NORMED, mask=self._mask)
